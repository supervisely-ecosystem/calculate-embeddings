    ...
    for dataset in datasets:
        ...
        for image_ids in sly.batched(to_infer_img_ids, batch_size=batch_size_api):
            ...
            for img_id, image, ann in zip(image_ids, images, annotations):
                new_embeds, crops_obj_cls, crops_yxyx = infer_one(image, ann.labels, instance_mode='both')
                if new_embeds is None: continue
                to_add_embeds.append(new_embeds)

                # add info for image
                upd = img_id2info[img_id].updated_at
                info = {'dataset_id': dataset.id, 'image_id': img_id, 'object_id': None, 'object_cls': None, 'crop_yxyx': crops_yxyx[0], 'updated_at': upd}
